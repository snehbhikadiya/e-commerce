...
const {notautherise}=require('../middleware/authorise');
...
routes.post('/register',notautherise,validation('User'),use(authController.register));


routes.get('/login',notautherise,use(authController.getlogin));
routes.post('/login',notautherise,use(passport.authenticate('local',{
